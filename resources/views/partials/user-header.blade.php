...
<header class="sebajar-user-header {{ $isHome ? 'sebajar-user-header--home' : '' }}" data-user-header>
    <div class="sebajar-user-nav">
        <a class="sebajar-user-logo" href="{{ url('/') }}" aria-label="Sebajar.id"><img src="{{ asset('images/sebajar-logo.png') }}" alt="Sebajar.id"></a>

        <nav class="sebajar-user-links" aria-label="Navigasi utama">
            <a class="{{ request()->routeIs('home') || request()->is('/') ? 'active' : '' }}" href="{{ url('/') }}">Beranda</a>
            <a class="{{ request()->routeIs('koleksi*') ? 'active' : '' }}" href="{{ route('koleksi') }}">Koleksi</a>
            <a href="{{ url('/#cara-sewa') }}">Cara Sewa</a>
            <a href="{{ url('/#tentang') }}">Tentang Kami</a>
            <a href="{{ $whatsappUrl }}" target="_blank" rel="noopener">Hubungi Admin</a>
        </nav>

        <div class="sebajar-user-actions">
            @if($isAuthenticated)
            <a class="user-order" href="{{ route('pesanan') }}" aria-label="Pesanan Saya">
                <span class="material-symbols-outlined">shopping_bag</span><span>Pesanan</span>
                @if($orderCount > 0)<span class="sebajar-badge">{{ $pendingOrderCount > 0 ? $pendingOrderCount : $orderCount }}</span>@endif
            </a>
            ...
            <div class="sebajar-profile">
                <button type="button" class="sebajar-profile-trigger" data-profile-toggle aria-expanded="false" aria-haspopup="true">
                    ...
                    <div class="sebajar-greeting"><small>Hi,</small><strong>{{ auth()->user()->name }}</strong></div>
                </button>
                <div class="sebajar-profile-menu" data-profile-menu>
                    <div class="sebajar-profile-info"><strong>{{ auth()->user()->name }}</strong><span>{{ auth()->user()->role }}</span></div>
                    <a href="{{ route('profil') }}"><span class="material-symbols-outlined">person</span>Profil</a>
                    <a href="{{ route('pesanan') }}"><span class="material-symbols-outlined">shopping_bag</span>Pesanan Saya</a>
                    <a href="{{ $whatsappUrl }}" target="_blank" rel="noopener"><span class="material-symbols-outlined">chat</span>Hubungi Admin</a>
            ...
            @else
                <a class="user-order" href="{{ route('login') }}">Masuk</a>
                <a class="user-order" href="{{ route('register') }}">Daftar</a>
            @endif
    ...
    <div class="sebajar-mobile-panel" data-mobile-panel>
        ...
        @if($isAuthenticated)
            ...
            <a href="{{ route('profil') }}"><span class="material-symbols-outlined">person</span> Profil Saya</a>
            <a href="{{ route('pesanan') }}"><span class="material-symbols-outlined">shopping_bag</span> Pesanan Saya @if($pendingOrderCount > 0)<span class="sebajar-mobile-badge">{{ $pendingOrderCount }}</span>@endif</a>
            <a href="{{ $whatsappUrl }}" target="_blank" rel="noopener"><span class="material-symbols-outlined">chat</span> Hubungi Admin</a>
            <form method="POST" action="{{ route('logout') }}">@csrf<button type="submit"><span class="material-symbols-outlined">logout</span> Keluar</button></form>
        @else
            <div class="sebajar-mobile-divider"></div>
