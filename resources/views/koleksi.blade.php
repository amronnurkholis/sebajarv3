    ...
    <main class="collection-content">
        ...
        <section class="rental-date-panel" aria-labelledby="rentalDateTitle">
            ...
            <div class="rental-date-control">
                ...
                <small id="rentalDateHint">
                    {{ $rentalDate ? 'Stok sudah dihitung untuk tanggal ini.' : 'Belum ada tanggal yang dipilih.' }}
                </small>
            </div>
        </section>
        ...
        <div class="collection-toolbar">
            ...
            <div class="result-count">
                Menampilkan
                <strong id="resultCount">{{ count($costumes) }}</strong>
                koleksi kostum
            </div>
        ...
        >
            ...
            @foreach ($costumes as $costume)
                ...
                <article
                    class="costume-card {{ $isOutOfStock ? 'is-unavailable' : '' }}"
                    data-name="{{ strtolower($name) }}"
                    data-code="{{ strtolower($code) }}"
                    data-status="{{ $isOutOfStock ? 'unavailable' : ($isLimited ? 'limited' : 'available') }}"
                >
                    ...
                    <div class="costume-image">
                        ...
                        @if ($imageUrl)
                            <img
                                src="{{ $imageUrl }}"
                                alt="Kostum Tari Saman {{ $code }} - {{ $name }}"
                                loading="lazy"
                            >
                        @else
                        ...
                        <span class="status {{ !$hasDate ? 'neutral' : ($isOutOfStock ? 'unavailable' : ($isLimited ? 'limited' : '')) }}">
                            {{ $displayStatus }}
                        </span>

                        <a href="{{ url('/koleksi/' . rawurlencode($code)) }}" class="costume-image-detail" aria-label="Lihat detail {{ $name }}">
                    ...
                    <div class="costume-body">
                        ...
                        <span class="costume-code">{{ $code }}</span>
                        ...
                        <div class="stock-context">{{ $rentalDate ? 'Ketersediaan untuk ' . \Carbon\Carbon::parse($rentalDate)->translatedFormat('d F Y') : 'Pilih tanggal untuk melihat stok aktual.' }}</div>
                        ...
                        <div class="costume-footer">
                            ...
                            <a
                                href="{{ url('/koleksi/' . rawurlencode($code)) }}"
                                class="detail-link"
                            >
                                Lihat Detail
                            ...
                            @if (!$isOutOfStock)
                                <a
                                    href="{{ url('/penyewaan?kostum=' . rawurlencode($code) . ($rentalDate ? '&rental_date=' . rawurlencode($rentalDate) : '')) }}"
                                    class="rent-link"
                                    data-rental-link
                                >
                                    Ajukan Sewa
