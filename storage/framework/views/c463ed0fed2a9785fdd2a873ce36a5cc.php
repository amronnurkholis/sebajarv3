...
<main class="orders-page"><div class="orders-wrap">
    <div class="orders-head">
        <div><a class="back-link" href="<?php echo e(url('/')); ?>">← Kembali ke beranda</a><h1>Pesanan Saya</h1><p>Semua pengajuan penyewaan yang terkait dengan akunmu.</p></div>
        <div class="orders-actions"><a href="<?php echo e(url('/koleksi')); ?>" class="btn btn-primary">Sewa Kostum</a></div>
    </div>
    <?php $__empty_1 = true; $__currentLoopData = $rentals; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rental): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <?php $statusClass = in_array($rental->status, ['pending','approved','completed','rejected'], true) ? $rental->status : 'pending'; ?>
        <article class="order-card">
            ...
            <div class="order-main-row">
                <div class="order-image">
                    <?php if($orderImage): ?>
                        <img src="<?php echo e($orderImage); ?>" alt="<?php echo e($rental->costume_code); ?>">
                    <?php else: ?>
                        <span class="material-symbols-outlined">checkroom</span>
                    <?php endif; ?>
                </div>
                <div class="order-content">
                    <div class="order-top"><div><div class="order-code"><?php echo e($rental->costume_code); ?></div><h2 class="order-title"><?php echo e($rental->team_name); ?></h2><div class="order-meta"><?php echo e($rental->quantity); ?> set · <?php echo e(optional($rental->rental_start)->format('d M Y')); ?></div></div><span class="order-status <?php echo e($statusClass); ?>"><?php echo e(match($rental->status){'pending'=>'Menunggu Persetujuan','approved'=>'Disetujui Admin','completed'=>'Selesai','rejected'=>'Ditolak',default=>ucfirst($rental->status)}); ?></span></div>
                    <div class="order-details"><div><div class="label">Nama penyewa</div><div class="value"><?php echo e($rental->customer_name); ?></div></div><div><div class="label">Ukuran dipilih</div><div class="value"><?php echo e($rental->sizes->map(fn($s) => ucfirst($s->category).' '.$s->size.' ('.$s->quantity.')')->join(', ') ?: '-'); ?></div></div></div>
                    <div class="order-actions-row"><a href="<?php echo e(config('sebajar.admin_whatsapp') ? 'https://wa.me/' . config('sebajar.admin_whatsapp') . '?text=' . rawurlencode('Halo Admin Sebajar, saya ingin menanyakan pesanan ' . $rental->costume_code . ' milik ' . $rental->customer_name . '.') : '#'); ?>" target="_blank" rel="noopener" class="order-contact"><span class="material-symbols-outlined" aria-hidden="true">chat</span> Hubungi Admin</a></div>
                </div>
            </div>
        </article>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
        <div class="empty-order"><span class="material-symbols-outlined">receipt_long</span><h2>Belum ada pesanan</h2><p>Pengajuan penyewaanmu akan muncul di sini setelah dikirim.</p><a href="<?php echo e(url('/koleksi')); ?>" class="text-link" style="margin-top:18px">Lihat Koleksi <span class="material-symbols-outlined" aria-hidden="true">arrow_forward</span></a></div>
    <?php endif; ?>
    <?php if($rentals->hasPages()): ?><div style="margin-top:20px"><?php echo e($rentals->links()); ?></div><?php endif; ?>
</div></main>
<?php echo $__env->make('partials.public-footer', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
</body>
</html>
<?php /**PATH /home/ophelia/Publik/sebajarv3/resources/views/pesanan/index.blade.php ENDPATH**/ ?>